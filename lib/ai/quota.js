...
// ⚠️ NINGUNA función de este módulo valida ownership por sí misma — ni las
// que usan serviceClient() (bypassa RLS por completo: checkAiQuota,
// recordAiUsage, checkEmailTestQuota, checkPlacesQuota) ni las que reciben
// un `supabase` de sesión pero escriben vía el RPC increment_ai_usage
// (SECURITY DEFINER, así que también bypassa RLS al escribir: getPetQuotaStatus,
// recordPetUsage). Todas confían en que quien las llama YA verificó que el
// userId/petId pertenece al usuario autenticado (ver ai-analyze/ai-symptoms/
// ai-recipe/ai-price-check, que hacen ese chequeo de ownership ANTES de
// invocar estas funciones). Si en el futuro se agrega un caller nuevo que no
// haga esa validación primero, queda expuesto a operar cuota/uso de
// cualquier usuario o mascota — hallazgo de la auditoría del Lote K1/K2.
...
// ── Cuota de detalle (teléfono/web) en /api/places/details ─────────────
// Tope más alto que la búsqueda porque cada click en una veterinaria es
// una llamada más barata a Google (Place Details de 1 lugar, no Nearby
// Search de ~20), y la caché en vet_details_cache evita repetir la
// llamada para la misma clínica entre usuarios.
export const PLACES_DETAILS_DAILY_LIMIT = 300;

export async function checkPlacesDetailsQuota(userId) {
  const supabase = serviceClient();
  const used = await getUserUsageToday(supabase, userId, "places_details");
  return { allowed: used < PLACES_DETAILS_DAILY_LIMIT, remaining: Math.max(0, PLACES_DETAILS_DAILY_LIMIT - used) };
}

// ── Cuota de invitaciones de tutor suplente distintas por día ──────────
// Evita que una cuenta use el envío de correos de invitación para
// mandar spam a direcciones arbitrarias cambiando el correo del tutor
// suplente repetidamente. No limita reenvíos al MISMO tutor (eso ya lo
// controla la ventana de 24 horas en /api/tutors/invite).
export const INVITE_DAILY_LIMIT = 10;

export async function checkInviteQuota(userId) {
  const supabase = serviceClient();
  const used = await getUserUsageToday(supabase, userId, "tutor_invite");
  return { allowed: used < INVITE_DAILY_LIMIT, remaining: Math.max(0, INVITE_DAILY_LIMIT - used) };
}
