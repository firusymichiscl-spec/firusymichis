...
export function computeItemNeeds(item, links, now = new Date()) {
  ...
  for (const link of links) {
    if (link.kind === "treatment") {
      ...
      // Lote T — un tratamiento condicional ("en caso de crisis") no tiene
      // un consumo diario real: "cada 12 horas" ahí describe la frecuencia
      // SI se da, no una toma garantizada. Se trata igual que un
      // medicamento habitual crónico (sin tasa, sin fin conocido) en vez de
      // llamar a dailyConsumptionForTreatment/getTreatmentProgress — nunca
      // aporta una fecha de agotamiento ni una sugerencia de compra falsas.
      if (ti.condicion) {
        anyOpenEnded = true;
        continue;
      }
