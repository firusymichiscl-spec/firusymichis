...
export default function EditPetModal({ pet, onClose, onSave }) {
  ...
  const css = {
    ...
    input: { width: "100%", padding: "10px 13px", borderRadius: 11, border: "1.5px solid #FFD9C8", background: "#fff", fontFamily: "'Nunito', sans-serif", fontSize: 14, color: "#3D1F0A", outline: "none" },
      ...
      <div style={css.modal}>
        ...
        <div style={css.speciesGrid}>
          {[{ value: "dog", icon: "🐶", label: "Perro" }, { value: "cat", icon: "🐱", label: "Gato" }, { value: "other", icon: "🐰", label: "Otro" }].map(s => (
            <div key={s.value} style={css.speciesBtn(form.species === s.value)}
        ...
        <button style={css.saveBtn} onClick={save} disabled={loading}>
