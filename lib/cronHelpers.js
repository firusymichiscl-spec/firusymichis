...
// puntuales). Sin dependencias — todo con Date/Intl nativos.
...
// start_time reales). Devuelve [] si el tratamiento no ha empezado, ya
// terminó, o la frecuencia es multi-día.
export function treatmentDoseTimesToday(ti, todayStr) {
  if (!ti.start_time || !ti.frequency || !ti.start_date) return [];
  if (todayStr < ti.start_date) return [];
  if (ti.duration_days) {
    const end = new Date(`${ti.start_date}T00:00:00`);
    end.setDate(end.getDate() + ti.duration_days);
    if (new Date(`${todayStr}T00:00:00`) >= end) return [];
  }
  ...
  const intervalMinutes = (24 * 60) / dpd;
  return Array.from({ length: Math.round(dpd) }, (_, i) => formatHM(startMinutes + i * intervalMinutes));
}
