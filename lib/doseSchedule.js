...
export function getPhaseSchedule(ti, horizon = new Date()) {
  // Lote T — un tratamiento condicional ("en caso de crisis", "S.O.S.") no
  // tiene un horario real que proyectar: su frecuencia describe qué pasa SI
  // se da, no una toma garantizada. Vista Fases llama a esta función
  // DIRECTO (sin pasar por getScheduledDoses), así que el guard tiene que
  // estar acá también, no solo ahí.
  if (ti?.condicion) return null;
...
export function getScheduledDoses(ti, rangeStart, rangeEnd) {
  // Lote T — condicional = sin dosis programadas (ver getPhaseSchedule más
  // arriba). Sin este guard, "cada 12 horas" seguiría generando 2 dosis/día
  // fantasma en Vista Hoy/Semana, en el backlog de "Ponerse al día" y en el
  // cálculo de adherencia (Lote L), para un tratamiento que solo se da si
  // ocurre la crisis.
  if (ti?.condicion) return [];
...
export function getTreatmentProgress(ti, now = new Date()) {
  // Lote T — condicional = sin progreso de horario que mostrar (ver
  // getScheduledDoses/getPhaseSchedule más arriba). Sin esto,
  // calcTreatmentProgress (DashboardClient.jsx) seguiría mostrando
  // "Próxima toma"/% completado como si fuera un tratamiento permanente.
  if (ti?.condicion) return null;
