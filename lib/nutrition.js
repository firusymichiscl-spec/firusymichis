...
import { getMaxAgeYears } from "./petSpecies";
...
const SPECIES_LABEL_AGE = { dog: "un perro", cat: "un gato", other: "esta mascota" };

// Devuelve { valid, message } — nunca lanza. Bloquea fechas futuras y
// edades que superan el máximo biológico razonable por especie/tipo
// (Fix 6 — antes se podía registrar una mascota de 118 años).
export function validateBirthDate(birthDate, species, breed) {
  if (!birthDate) return { valid: true, message: null };
  const birth = new Date(birthDate + "T00:00:00");
  if (isNaN(birth.getTime())) return { valid: true, message: null };
  const now = new Date();
  if (birth > now) {
    return { valid: false, message: "La fecha de nacimiento no puede ser futura." };
  }
  const ageYears = (now - birth) / (365.25 * 86400000);
  const maxAge = getMaxAgeYears(species, breed);
  if (ageYears > maxAge) {
    return {
      valid: false,
      message: `La edad de ${SPECIES_LABEL_AGE[species] || "esta mascota"} no puede superar los ${maxAge} años. Verifica la fecha de nacimiento.`,
    };
  }
  return { valid: true, message: null };
}

// Convierte una edad aproximada en años (puede tener decimales, ej. 1.5)
// en una fecha de nacimiento estimada, para cuando el veterinario dio una
// edad aproximada en vez de fecha exacta (mascota adoptada o rescatada).
// Se ancla a HOY menos esos meses, para que la edad calculada a partir de
// esta fecha siga siendo correcta a medida que pasa el tiempo.
export function birthDateFromApproxYears(years) {
